...
// --------------------------------------------------------------------
// Get values of many variable s
//
exports.query = function(req, res, next) {
	debug("api.query for ", req.body.var_list);
    var data = { status : "OK",
                errors  : []
               };
    ...
    variables.query(req.body, function(err, values) {
	    debug("===> return  form query");
	    debug("      err = " + err);
	    debug("      values = ", values);
        if (err) {
            data.status = "KO";
...
exports.get_last_value = function(req, res, next) {
    ...
    var v = variables.get_variable(req.query.ident);
	debug("===> return  form get_varable = ", v);
    if (v) {
         data.ident = req.query.ident;
         data.meta = v.meta;
    }
    else {
        data.status = "KO";
	    debug("===> ERROR  form get_varable = ", v);
    }
    res.send(data);
};
// --------------------------------------------------------------------
