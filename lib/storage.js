...
// --------------------------------------------------------------------
exports.get_variable_values = function(ident, start_date, end_date, cb) {
    debug(" storage.get_variable_values " + ident);

    var dir_path = config.files_dir + "variables/" + ident;
	debug(" dir_path = " + dir_path);
...
/*
    ...
    var cmd = child.spawn('/bin/sh', ['-c', 'cat ' + dir_path + '/[0-9]* | sort']);
    ...
    cmd.on('close', function(code) {
        debug("processus end code = " + code);
        if (code === 0) {
            cb(null, str);
        }
        else {
            cb("Error getting values from ident (code = " + code + ")", null);
        }
    });
*/
}
...
exports.read_screens = function() {
    ...
    var dirs = fs.readdirSync(base_path);
    dirs.forEach(function(item) {
        var ident = item.replace(/.yml$/, "");
        debug("item = " + item + " ident = " + ident);
        var path = base_path + "/" + item;
        var screen = yaml.load(path);
        if (screen) {
            data[ident] = yaml.load(path);
        }
        else {
            // TODO log error
	        debug(" screen id null");
        }
    });
