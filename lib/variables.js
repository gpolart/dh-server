...
exports.get_values = function(ident, start_date, end_date, cb) {
    ...
    if (!variables.hasOwnProperty(ident)) { 
        ...
        if (start > end) {
            ...
            storage.get_variable_values({ ident: ident,
                            start : start,
                            end : end,
                            type: variables[ident].meta.type,
                            }, cb);
        }
    }
};
// --------------------------------------------------------------------
// Query values for multiple variable
//
exports.query = function(params, cb) {
	debug("variables.query");

    var funcs = [];
    var errors = [];

    var start_date = new Date();
    start_date.parseDate(params.start_date);
    var end_date = new Date();
    end_date.parseDate(params.end_date);

    params.var_list.forEach(function(item, num) {
        if (!variables.hasOwnProperty(item)) { 
            errors.push("Error : the variable " + item + "doesn't exist");
        }
        else {
            if (variables[item].meta.type !== params.type) {
                errors.push("Error : the variable " + item + " has wrong type");
            }
            else {
                funcs.push(function(callback) {
                    debug("query for " + item + " from " + params.start_date + " to " + params.end_date);
                    storage.get_variable_values({ ident: item,
                                    start : start_date,
                                    end : end_date,
                                    type: params.type,
                                    }, callback);
    ...
    if (errors.length > 0) {
       cb(errors, null);
        ...
        async.parallel(funcs, function(err, result) {
            debug("parallel end err = ", err)
            debug("parallel end result = ", result)
            cb(err, result);
        });
    }
};
